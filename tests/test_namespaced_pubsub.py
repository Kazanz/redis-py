...
class TestPubSubSubscribeUnsubscribe(object):
    ...
    def test_channel_subscribe_unsubscribe(self, nr):
        kwargs = make_subscribe_test_data(nr.pubsub(), 'channel')
        self._test_subscribe_unsubscribe(**kwargs)

    def test_pattern_subscribe_unsubscribe(self, nr):
        kwargs = make_subscribe_test_data(nr.pubsub(), 'pattern')
        self._test_subscribe_unsubscribe(**kwargs)
    ...
    def test_resubscribe_to_channels_on_reconnection(self, nr):
        kwargs = make_subscribe_test_data(nr.pubsub(), 'channel')
        self._test_resubscribe_on_reconnection(**kwargs)

    def test_resubscribe_to_patterns_on_reconnection(self, nr):
        kwargs = make_subscribe_test_data(nr.pubsub(), 'pattern')
        self._test_resubscribe_on_reconnection(**kwargs)
    ...
    def test_subscribe_property_with_channels(self, nr):
        kwargs = make_subscribe_test_data(nr.pubsub(), 'channel')
        self._test_subscribed_property(**kwargs)

    def test_subscribe_property_with_patterns(self, nr):
        kwargs = make_subscribe_test_data(nr.pubsub(), 'pattern')
        self._test_subscribed_property(**kwargs)

    def test_ignore_all_subscribe_messages(self, nr):
    ...
    def test_ignore_individual_subscribe_messages(self, nr):
        p = nr.pubsub()
...
class TestPubSubMessages(object):
    ...
    def test_published_message_to_channel(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
        p.subscribe('foo')
        assert nr.publish('foo', 'test message') == 1

        message = wait_for_message(p)
        assert isinstance(message, dict)
        assert message == make_message('message', 'foo', 'test message')

    def test_published_message_to_pattern(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
        p.subscribe('foo')
        p.psubscribe('f*')
        # 1 to pattern, 1 to channel
        assert nr.publish('foo', 'test message') == 2
    ...
    def test_channel_message_handler(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
    ...
    def test_unicode_channel_message_handler(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
        channel = u('uni') + unichr(4456) + u('code')
        channels = {channel: self.message_handler}
        p.subscribe(**channels)
        assert nr.publish(channel, 'test message') == 1
        assert wait_for_message(p) is None
        assert self.message == make_message('message', channel, 'test message')

    def test_unicode_pattern_message_handler(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
    ...
    def test_get_message_without_subscribe(self, nr):
        p = nr.pubsub()
...
class TestPubSubAutoDecoding(object):
    ...
    def test_channel_publish(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
        p.subscribe(self.channel)
    ...
    def test_pattern_publish(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
        p.psubscribe(self.pattern)
        nr.publish(self.channel, self.data)
...
class TestPubSubPubSubSubcommands(object):
    ...
    def test_pubsub_channels(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
        p.subscribe('foo', 'bar', 'baz', 'quux')
        channels = sorted(nr.pubsub_channels())
        assert channels == [b('bar'), b('baz'), b('foo'), b('quux')]

    @skip_if_server_version_lt('2.8.0')
    def test_pubsub_numsub(self, nr):
        p1 = nr.pubsub(ignore_subscribe_messages=True)
        p1.subscribe('foo', 'bar', 'baz')
        p2 = nr.pubsub(ignore_subscribe_messages=True)
        p2.subscribe('bar', 'baz')
        p3 = nr.pubsub(ignore_subscribe_messages=True)
        p3.subscribe('baz')

        channels = [(b('foo'), 1), (b('bar'), 2), (b('baz'), 3)]
        assert channels == nr.pubsub_numsub('foo', 'bar', 'baz')

    @skip_if_server_version_lt('2.8.0')
    def test_pubsub_numpat(self, nr):
        p = nr.pubsub(ignore_subscribe_messages=True)
        p.psubscribe('*oo', '*ar', 'b*z')
        assert nr.pubsub_numpat() == 3
